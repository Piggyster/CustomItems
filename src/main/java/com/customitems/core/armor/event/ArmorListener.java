package com.customitems.core.armor.event;
...
public class ArmorListener implements Listener {
    ...
    @EventHandler
    public void onEvent(ArmorUnequipEvent event) {
        Item item = Item.of(event.getItem());
        if(item == null) return;
        event.getPlayer().sendMessage(event.getAction().toString());
        //event.getItem().setType(Material.DIRT);
        item.update(event.getPlayer(), event.getItem());
    }
            ...
            for (int i = armor.length - 1; i >= 0; i--) {
                if (holding.isSimilar(armor[i])) {
                    ...
                    if (armorEvent.isCancelled()) {
                        ...
                        Bukkit.getScheduler().runTask(ArmorPlugin.getPlugin(), () -> {
